...
description: >
  ...
  hub's `_CLOSETS.md` / `_CLOSETS-archive.md` plus `memory/_CLOSETS.md`, and
  queries `memory/.facts.db`. Results are grouped by folder so cross-domain
  queries (e.g., "everywhere I mentioned Airtable", "decisions touching the
  spring campaign") surface every hub that has a hit. Use when the question
  spans hubs, when you don't know which hub owns the answer, or when grep
  across the index files is the right tool. For cross-workspace search, use
  `/memex:cross-search`. Reads only manifest, closets, and facts.db — never
...
If `$ARGUMENTS` includes `--no-facts`, the temporal facts query is skipped (manifest + closets only). Default: facts on.
...
Add `--no-facts` if the user passed it.

The script greps `_MANIFEST.md` plus every `_CLOSETS.md` and `_CLOSETS-archive.md` under the workspace, plus queries `memory/.facts.db` for currently-valid facts (`valid_to IS NULL`) whose subject, predicate, or object matches. Output is grouped by folder.
...
- Zero closets hits, only facts.db hits → "Tracked as a temporal fact; no closets coverage yet. Consider adding a file or amending an existing one so future sessions retrieve it via the closets path."

---

## Gotchas

- **Index-only search.** Manifest, closets, facts.db. Never inside content files. If a subject isn't wikilinked into a closet, `/memex:search` won't see it. The fix is `/memex:resummarize` or `/memex:reindex`, not deeper grep.
- **Currently-valid facts only** (`valid_to IS NULL`). Superseded facts don't surface here. Use `/memex:facts timeline <subject>` for full history within a single subject.
- **Case-insensitive substring.** Regex chars are escaped automatically; you can't use regex.
- **Search is read-only.** No writes, no closets refresh, no facts mutations. To fix gaps surfaced by a search, run `/memex:reindex` (closets) or `/memex:facts add` (facts).
