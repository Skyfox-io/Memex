...
For the v1 → v2 upgrade specifically, see [`references/v1-to-v2.md`](references/v1-to-v2.md). Future major versions add their own playbooks under `references/`.
...
Read four signals from the workspace:
...
3. **Closets coverage.** For each `[[*-index]]` row in the Hub Map, check whether `<hub>/_CLOSETS.md` exists. Count hubs with coverage vs hubs without.
