...
name: wikilinks
description: >
  Verifies wikilink integrity (broken `[[targets]]`), then optionally suggests
  plain-text-to-wikilink conversions across the workspace. The conversion sweep is
  a workspace-wide bulk edit, so review suggestions before accepting. For a broader
  workspace audit (orphans, stale status, manifest drift), use `/memex:lint`.
