...
- **Built-in similarity search.** `/memex:search` supplements grep with a scored fuzzy/BM25-style fallback, out of the box, zero deps — catches morphological variants grep's substring match can't (fundraiser/fundraising). Auto-upgrades to embedding-based paraphrase matching where `sentence-transformers` already exists, no install or config needed either way.
